...
A character has three kinds of assets, each secured differently, and a contract can never hold a secret because everything on-chain is public. The master key is the aNFT; every door asks, do you hold it. **Own** (money on Sui): objects the aNFT owns directly, no key. **Read** (memory, persona, read-class saved credentials that cannot move money): encrypted, released for off-chain decryption only to proven ownership or a guardian majority. **Spend on other chains**: the target is MPC dWallets (Ika) — never a stored key; a network holds it in shares never assembled, and ownership authorizes signing. The rule: if it can be read, decrypt it; if it can spend, authorize it, never decrypt it. Sort every secret by what a leaked copy would cost, not by whether it is secret: a revocable read-class token goes behind Seal; anything that can move value is spend-class and never does.
...
The endowment holds a principal (stablecoin) that earns yield, plus a small liquid buffer, split into ring-fenced allocations (keep-alive, storage, interaction) that cannot drain one another. A permissionless heartbeat collects yield each cycle, distributes it in priority order, pays due costs, and compounds the rest. The owner sets a keep-alive reserve that withdrawals can never breach. Fed by yield and by earned income, with compute always covered first. Sized against adverse yield, never a promised return. Full treatment in Funding a Character.
...
A character works in text, voice (messages and real-time calls), image, and video; the owner chooses which are on and how they are priced. Billing is a meter over real cost drivers, not a flat list: every interaction returns a usage record (tokens, audio-seconds or session length, image resolution and reference count, video length and resolution), and settlement is true cost plus a routing fee. Predictable actions are quoted up front; variable or session actions run against a pre-authorized hold and settle the actual amount. Compute is always covered before anything is kept. Fans see credits or a pass, never tokens.
