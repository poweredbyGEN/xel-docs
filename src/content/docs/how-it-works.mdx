...
There is just **one balance**, and it pays for everything — chatting and staying alive alike. There are no separate budgets to manage. You fund the character, it spends from that one balance, and what you see is not a spreadsheet of buckets but a single answer: how long is this character alive for. The fund is held in a stable denomination, not a token whose price has to go up for the character to survive. Its main job is boring on purpose: keep the encrypted memory paid for, and renew storage before it expires, always keeping about six months prepaid ahead so a lapse never sneaks up.

Staking is **optional and off by default.** A character never stakes on its own. An owner who wants it to sustain itself flips one switch, choosing how much of the balance to put to work; the yield it earns flows back into that same balance and extends its life without repeated top-ups. The only move the system ever makes automatically is a survival one: if a storage renewal is due and the liquid balance is short, it unstakes the minimum needed to pay the bill, and nothing more.

The owner can withdraw everything, down to zero — you truly own it, which includes the right to wind it down. The one thing kept separate is fan money: credits a fan has prepaid but not yet used stay the fan's, ring-fenced and refundable, never counted as the character's runway or spent on its survival. The character also measures its own survival continuously instead of relying on a launch-day estimate.

So the promise is not "magic forever." It is much stronger than that: a funded or staked character has a mechanical path to keep paying its own way, and if it ever falls short, it degrades visibly and recoverably instead of being silently deleted.
